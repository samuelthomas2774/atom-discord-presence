...
export default class {
    ...
    setJoinSecret(joinSecret) {
        if (this.joinSecret === joinSecret) return;
        this.joinSecret = joinSecret;
        this.emitter.emit('did-change-join-secret', joinSecret);
        this.emitter.emit('did-activity-update');
    }

    onDidChangeJoinSecret(callback) {
        return this.emitter.on('did-change-join-secret', callback);
    }

    observeJoinSecret(callback) {
        return callback(this.joinSecret), this.onDidChangeJoinSecret(callback);
    }
    ...
    getActivity() {
        ...
        if (AtomDiscordPresence.privacy.sendTeletypeToken) {
            const joinSecret = this.getJoinSecret();
            if (joinSecret) data.joinSecret = joinSecret;
        }
